...
pub async fn run(cli: Cli) -> Result<i32> {
    ...
    match cli.command {
        ...
        Some(Commands::Test { pattern }) => {
            build::test(pattern, &output).await
        }
        Some(Commands::Run { args }) => {
            run::run(args, &output).await
        }
        Some(Commands::Repl) => {
            run::repl(&output).await
        }
        Some(Commands::Check) => {
        ...
        Some(Commands::Fmt { check }) => {
            fmt::run(check, &output).await
        }
        Some(Commands::Lint { fix }) => {
            lint::run(fix, &output).await
        }
        Some(Commands::Doctor) => {
            doctor::run(&output).await
        }
        Some(Commands::Lock) => {
            lock::run(&output).await
        }
        Some(Commands::Sync { force }) => {
            lock::sync(force, &output).await
        }
        Some(Commands::Clean { global }) => {
            clean::run(global, &output).await
        }
